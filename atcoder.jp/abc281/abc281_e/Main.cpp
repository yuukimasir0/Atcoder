...
template<class T> using o_set = tree<T, null_type, less<>, rb_tree_tag, tree_order_statistics_node_update>;
...
struct Solver {
    void solve() {
        ...
        VC(i64, A, N);
        rep(M) {
            st.insert(pair(A[i], i));
        }
        rep(K) ans += (*st.find_by_order(i)).first;
        print(ans);
        if(K == M) {
            rep(N - M) {
                ans -= A[i];
        ...
        rep(N - M) {
            if(A[i] <= (*st.find_by_order(K - 1)).first) {
                ans += (*st.find_by_order(K)).first;
                ans -= A[i];
            }
            st.erase(pair(A[i], i));
            if(A[M + i] < (*st.find_by_order(K - 1)).first) {
                ans += A[M + i];
                ans -= (*st.find_by_order(K - 1)).first;
            }
            st.insert(pair(A[M + i], M + i));
            print(ans);
        }
    }
