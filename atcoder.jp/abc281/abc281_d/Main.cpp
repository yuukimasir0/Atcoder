...
struct Solver {
    void solve() {
        INT(N, K, D);
        VC(i64, A, N);
        vector dp(N + 1, vector<vector<i64>>(K + 1, vector<i64>(D, -1)));
        dp[0][0][0] = 0;
        rep(i, N) rep(k, K) rep(d, D) {
            chmax(dp[i + 1][k + 1][d], dp[i][k + 1][d]);
            chmax(dp[i + 1][k][d], dp[i][k][d]);
            if(dp[i][k][d] != -1) chmax(dp[i + 1][k + 1][(d + A[i]) % D], dp[i][k][d] + A[i]);
        }
        print(dp[N][K][0]);
    }
