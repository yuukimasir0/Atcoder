...
struct Solver {
    void solve() {
        LL(H, W);
        i64 ans = 0;
        vv<char> S(H + 1, vc<char>(W + 1));
        rep(i, 1, H + 1) rep(j, 1, W + 1) input(S[i][j]);
        vc<vv<i32>> g(H + 1, vv<i32>(W + 1));
        vc<pair<i32, i32>> d = {pair(0, 1), pair(0, -1), pair(1, 0), pair(-1, 0)};
        rep(i, 1, H + 1) rep(j, 1, W + 1) {
            if(S[i][j] == '#') continue;
            ans++;
            for(const auto &[dx, dy] : d) {
                if(i + dx > 0 && i + dx <= H && j + dy > 0 && j + dy <= W && S[i + dx][j + dy] == '.') {
                    g[i][j].push_back((i + dx) * 100 + j + dy);
                    g[i + dx][j + dy].push_back(i * 100 + j);
                }
            }
        }
        ...
        while(q.size()) {
            auto r = q.front();
            q.pop();
            auto x = r / 100, y = r % 100;
            for(const auto &u : g[x][y]) {
                auto ux = u / 100, uy = u % 100;
                if(chmin(dist[ux][uy], dist[x][y] + 1))
                    q.push(u);
            }
        }
        debugvc(dist);
        if(dist[H][W] == INT32_MAX) { println("-1"); }
        ans -= dist[H][W];
        print(ans);
    }
};
