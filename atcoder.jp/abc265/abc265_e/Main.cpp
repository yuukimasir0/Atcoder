...
template<class T> ostream &operator<<(ostream &i, vc<T> &v) {
    for(auto &&j : v) cout << j << ' ';
    cout << '\n';
    return i;
}
...
struct Solver {
    void solve() {
        ...
        dp[{0, 0}] = 1;
        rep(N) {
            map<pair<i64, i64>, i64> temp_dp;
            for(const auto &[xy, val] : dp) {
                const auto &[x, y] = xy;
                if(st.find({x + A, y + B}) == st.end()) (temp_dp[{x + A, y + B}] += val) %= mod;
                if(st.find({x + C, y + D}) == st.end()) (temp_dp[{x + C, y + D}] += val) %= mod;
                if(st.find({x + E, y + F}) == st.end()) (temp_dp[{x + E, y + F}] += val) %= mod;
            }
            dp = move(temp_dp);
        }
        i64 ans = 0;
        for(const auto &[_, val] : dp) (ans += val) %= mod;
        print(ans);
    }
};
