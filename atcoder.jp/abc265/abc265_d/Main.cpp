...
struct Solver {
    void solve() {
        ...
        rep(N) {
            auto x = b.sum(i);
            if(b.find(x + P) == false) continue;
            if(b.find(x + P + Q) == false) continue;
            if(b.find(x + P + Q + R) == false) continue;
            println("Yes");
        }
        print("No");
    }
