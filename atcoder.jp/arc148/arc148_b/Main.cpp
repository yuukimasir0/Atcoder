...
struct Solver {
    void solve() {
        ...
        rep(i, key, N) {
            if(S[i] == 'd') continue;
            string tmp = S.substr(0, key);
            string T = S.substr(key, i - key + 1);
            reverse(all(T));
            rep(j, T.size()) if(T[j] == 'p') T[j] = 'd'; else T[j] = 'p';
            int dbg = tmp.size(), dbg2 = T.size();
            // debug(key, i, dbg, dbg2, T, "");
            tmp = tmp + T + S.substr(i + 1);
            debug(tmp, "\n");
            ans.push_back(tmp);
        }
        sort(all(ans));
