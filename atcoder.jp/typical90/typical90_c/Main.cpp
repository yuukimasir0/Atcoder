...
struct Solver {
    void solve() {
        ...
        while(q.size()) {
            auto v = q.front();
            q.pop();
            each(u, G[v]) if(chmin(dist[u], dist[v] + 1)) q.push(u);
        }
        int s = 0;
        int tmp = 0;
        rep(N + 1) if(dist[i] > tmp) {
            s = i;
            tmp = dist[i];
        }
        q.push(s);
        vc(int, ans, N + 1, INT_MAX);
        ans[0] = ans[s] = 0;
        while(q.size()) {
            auto v = q.front();
            q.pop();
            each(u, G[v]) if(chmin(ans[u], ans[v] + 1)) q.push(u);
        }
        auto res = max(ans) + 1;
        print(res);
    }
