...
struct Solver {
    void solve() {
        ...
        vc(i64, P, 1, 2);
        int n = 92e4;
        P.reserve((int)sqrt(n));
        vc(bool, check, n, true);
        check[0] = check[1] = false;
        for(i64 i = 9; i < n; i += 3) check[i] = false;
        for(i64 i = 0; (6 * i + 1) * (6 * i + 1) < n; i++) {
            if(check[6 * i + 1])
                for(i64 j = (6 * i + 1) * (6 * i + 1); j < n; j += (6 * i + 1)) check[j] = false;
            if(check[6 * i + 5])
                for(i64 j = (6 * i + 5) * (6 * i + 5); j < n; j += (6 * i + 5)) check[j] = false;
        }
        rep(i, 3, n, 2) if(check[i]) P.push_back(i);
        i64 ans = 0;
        n = P.size();
        rep(n) {
            rep(j, i + 1, n) {
