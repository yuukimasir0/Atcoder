...
struct Solver {
    void solve() {
        ...
        rep(i, 1, N + 1) {
            input(P[i]);
            if(i % 2 != P[i] % 2) st.insert(i);
        }
        if(is_sorted(all(P))) println("0");
        while(st.size()) {
            rep(st.size()) {
                if(i == st.size() - 1) break;
                auto tmp1 = st.find_by_order(i + 1);
                auto tmp2 = st.find_by_order(i);
                if(*tmp1 - *tmp2 == 1) {
                    ans.push_back(pair('A', *tmp2));
                    swap(P[*tmp1], P[*tmp2]);
                    st.erase(tmp1);
                    st.erase(tmp2);
                    i--;
            ...
            rep(_i, st.size()) {
                if(_i == st.size() - 1) break;
                auto tmp1 = st.find_by_order(_i + 1);
                auto tmp2 = st.find_by_order(_i);
                auto cnt = *tmp1 - *tmp2;
                if(cnt % 2 == 1) {
                    cnt /= 2;
                    int n = *tmp2;
                    rep(cnt) {
                        ans.push_back(pair('B', n + 2 * i));
                        swap(P[n + 2 * i], P[n + 2 * i + 2]);
                    }
                    swap(P[*tmp1 - 1], P[*tmp1]);
                    ans.push_back(pair('A', *tmp1 - 1));
                    st.erase(tmp1);
                    st.erase(tmp2);
                    _i--;
                }
            }
            rep(i, 1, N + 1) if(i % 2 != P[i] % 2) st.insert(i);
        }
        //ここまでパリティを揃える操作
        //ここからバブルソート
        rep(i, 1, N + 1) rep(j, 1, N - 1) {
            if(P[j + 2] < P[j]) {
                swap(P[j], P[j + 2]);
                ans.push_back(pair('B', j));
            }
        }
        print(ans.size());
        debugvc(P);
        debug("\n");
        each(i, j, ans) print(i, j);
    }
};
