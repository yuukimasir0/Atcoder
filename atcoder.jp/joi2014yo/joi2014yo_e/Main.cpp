...
struct Solver {
    void solve() {
        ...
        vv<i32> dist(N + 1, vc<i32>(N + 1, INT32_MAX));
        auto init = [&]() {
            rep(x, 1, N) {
                dist[x][x] = 0;
                queue<i32> q;
                ...
                while(q.size()) {
                    ...
                    for(const auto &u : g[v])
                        if(chmin(dist[x][u], dist[x][v] + 1)) q.push(u);
                }
            }
            rep(i, 1, N) rep(j, 1, N + 1) if(dist[i][j] <= R[i] && dist[i][j] > 1) g[i].push_back(j);
        };
        init();
        pq<i32> q;
        q.push(1);
