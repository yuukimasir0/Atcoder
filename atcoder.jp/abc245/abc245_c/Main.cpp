...
#define vv1(class, name, H) std::vector name(H, std::vector<class>())
#define vv2(class, name, H, ...) std::vector name(H, std::vector<class>(__VA_ARGS__))
#define vv(...) overload5(__VA_ARGS__, vv2, vv2, vv1)(__VA_ARGS__)
...
struct Solver {
    void solve() {
        ...
        VC(int, B, N);
        rep(i, 1, N) {
            if(dp[i][0] && abs(A[i] - A[i - 1]) <= K) dp[i + 1][0] = true;
            if(dp[i][1] && abs(A[i] - B[i - 1]) <= K) dp[i + 1][0] = true;
            if(dp[i][0] && abs(B[i] - A[i - 1]) <= K) dp[i + 1][1] = true;
            if(dp[i][1] && abs(B[i] - B[i - 1]) <= K) dp[i + 1][1] = true;
        }
        printif(dp[N][0] || dp[N][1], "Yes", "No");
    }
};
