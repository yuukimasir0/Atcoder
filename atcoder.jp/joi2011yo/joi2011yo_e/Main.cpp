...
struct Solver {
    void solve() {
        LL(H, W, N);
        vv<char> M(H + 1, vc<char>(W + 1));
        vc<pair<i32, i32>> F(N + 1);
        vc<pair<i32, i32>> d = {
            pair(1, 0),
            pair(-1, 0),
            pair(0, 1),
        ...
        rep(i, 1, H + 1) rep(j, 1, W + 1) {
            input(M[i][j]);
            if(M[i][j] >= '1' && M[i][j] <= '9') F[M[i][j] - '0'] = pair(i, j);
            if(M[i][j] == 'S') F[0] = pair(i, j);
        }
        vc<vv<pair<i32, i32>>> g(H + 1, vv<pair<i32, i32>>(W + 1));
        rep(i, 1, H + 1) rep(j, 1, W + 1) {
            for(auto &[dx, dy] : d) {
                if(M[i][j] == 'X') continue;
                if(i + dx > 0 && i + dx <= H && j + dy > 0 && j + dy <= W && M[i + dx][j + dy] != 'X') {
                    g[i][j].emplace_back(pair(i + dx, j + dy));
                    g[i + dx][j + dy].emplace_back(pair(i, j));
                }
            }
        }
        i64 ans = 0;
        rep(start, N) {
            vv<i32> dist(H + 1, vc<i32>(W + 1, INT32_MAX));
            queue<pair<i32, i32>> q;
            q.push(F[start]);
            dist[F[start].first][F[start].second] = 0;
            while(q.size()) {
                auto &[vx, vy] = q.front();
                q.pop();
                for(auto &[ux, uy] : g[vx][vy]) {
                    if(chmin(dist[ux][uy], dist[vx][vy] + 1))
                        q.push(pair(ux, uy));
                }
            }
            ans += dist[F[start + 1].first][F[start + 1].second];
        }
        println(ans);
    }
};
