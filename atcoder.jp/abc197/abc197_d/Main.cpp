...
template<class T, class U> pair<T, U> &operator+(pair<T, U> p1, pair<T, U> p2) {pair<T, U> res = pair(p1.first + p2.first, p1.second + p2.second); return res;};
...
struct Solver {
    void solve() {
        ...
        vc(pld, r, N);
        input(r[0], r[N / 2]);
        pld d = pair((r[0].first - r[N / 2].first) / 2, (r[0].second - r[N / 2].second) / 2);
        pld c = pair(r[N / 2].first + d.first, r[N / 2].second + d.second);
        ld x = r[0].first - c.first;
        ld y = r[0].second - c.second;
        ld theta = 2 * acos(-1) / N;
        pld ans = pair(x * cos(theta) - y * sin(theta) + c.first, x * sin(theta) + y * cos(theta) + c.second);
        debug(x, y, "\n");
        debug(theta, c, "\n");
        print(ans);
    }
