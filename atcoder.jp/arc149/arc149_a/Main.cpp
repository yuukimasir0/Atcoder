...
struct Solver {
    void solve() {
        ...
        vv(int, A, N, 10);
        rep(10) A[0][i] = i % M;
        rep(i, 1, N) rep(j, 1, 10) A[i][j] = 10 * A[i - 1][j] % M;
        pint ans = pair(0, 0);
        rep(j, 1, 10) {
            auto tmp = 0;
            rep(N) {
                (tmp += A[i][j]) %= M;
                if(tmp == 0 && ans.first <= i) ans = pair(i, j);
            }
        }
        if(ans == pair(0, 0))
            cout << -1;
        else
            rep(ans.first + 1) cout << ans.second;
        cout << '\n';
    }
    ...
    void aaaaaaaaaaaaaa() {
        ...
        pint ans = pair(0, 0);
        rrep(N) {
            rep(j, 1, 10) if(f(i, j)) ans = pair(i + 1, j);
            if(ans != pair(0, 0)) break;
        } //O(N * N)
        if(ans == pair(0, 0))
            print("-1");
        else
            rep(ans.first) cout << ans.second;
        cout << '\n';
    }
};
