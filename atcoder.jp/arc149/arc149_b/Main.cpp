...
struct Solver {
    void solve() {
        ...
        int lena = 0, lenb = 0;
        rep(N) {
            int ad = lower_bound(La.begin() + 1, La.begin() + lena + 1, sA[i].second) - La.begin();
            dpa[i] = ad;
            int bd = lower_bound(Lb.begin() + 1, Lb.begin() + lenb + 1, sB[i].second) - Lb.begin();
            dpb[i] = bd;
            La[dpa[i]] = sA[i].second;
            Lb[dpb[i]] = sB[i].second;
            if(dpa[i] > lena) lena++;
            if(dpb[i] > lenb) lenb++;
        }
        ans += max(lena, lenb);
