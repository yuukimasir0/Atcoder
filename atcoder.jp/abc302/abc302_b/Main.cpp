...
struct Solver {
    void solve() {
        INT(H, W);
        VC(string, S, H);
        pair<int, int> d[] = {pair(-1, -1), pair(-1, 0), pair(-1, 1), pair(0, -1), pair(0, 1), pair(1, -1), pair(1, 0), pair(1, 1)};
        string snuke = "snuke";
        auto f = [&](int _x, int _y) {
            each(dx, dy, d) {
                int x = _x, y = _y;
                bool flg = true;
                for(int i = 0; i < 5 && flg; i++) {
                    if(x < 0 || y < 0 || x >= H || y >= W) flg = false;
                    if(flg && S[x][y] != snuke[i]) flg = false;
                    x += dx, y += dy;
                }
                if(flg) {
                    x = _x, y = _y;
