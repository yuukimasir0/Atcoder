...
struct Solver {
    void solve() {
        ...
        rep(M) {
            LL(A, B);
            g[A].emplace_back(pair{0, B});
            g[B].emplace_back(pair{0, A});
        }
        auto init = [&](i64 x) {
            vc<i64> dist(N + 1, LLONG_MAX);
            queue<i64> q;
            q.push(x);
            dist[x] = 0;
            while(q.size()) {
            ...
            rep(i, 1, N + 1) if(dist[i] <= S) cost[i] = Q;
        };
        for(const auto &x : C) init(x);
        rep(K) cost[C[i]] = LLONG_MAX;
        cost[N] = 0;
        rep(i, 1, N + 1) for(auto &&[w, x] : g[i]) w = cost[x];
        pq<i64> q;
        vc<i64> ans(N + 1, LLONG_MAX);
        ans[1] = 0;
        q.push(1);
        while(q.size()) {
            auto v = q.top();
