...
// clang-format off
    ...
    #define END(...) { print(__VA_ARGS__); exit(0); }
...
struct Solver {
    ...
    AAA:
        auto dfs = [&](auto dfs, pint before, pint r) -> void{
            auto &[x, y] = r;
            debug(x, y, "\n");
            if(x != 0 && C[x - 1][y] == '.' && check[x - 1][y] == 0) {
                check[x - 1][y] = 1;
                dfs(dfs, r, pair(x - 1, y));
            }
            if(y != 0 && C[x][y - 1] == '.' && check[x][y - 1] == 0) {
                check[x][y - 1] = 1;
                dfs(dfs, r, pair(x, y - 1));
            }
            if(x != H - 1 && C[x + 1][y] == '.' && check[x + 1][y] == 0) {
                check[x + 1][y] = 1;
                dfs(dfs, r, pair(x + 1, y));
            }
            if(y != W - 1 && C[x][y + 1] == '.' && check[x][y + 1] == 0) {
                check[x][y + 1] = 1;
                dfs(dfs, r, pair(x, y + 1));
            }
            ...
            if(x != 0 && C[x - 1][y] == 'S') {
                check[x - 1][y] = 1;
                if(before != S && x - 1 == S.first && y == S.second && (abs(r.first - before.first) + abs(r.second - before.second) <= 1)) END("Yes");
            }
            if(y != 0 && C[x][y - 1] == 'S') {
                check[x][y - 1] = 1;
                if(before != S && x == S.first && y - 1 == S.second && (abs(r.first - before.first) + abs(r.second - before.second) <= 1)) END("Yes");
            }
            if(x != H - 1 && C[x + 1][y] == 'S') {
                check[x + 1][y] = 1;
                if(before != S && x + 1 == S.first && y == S.second && (abs(r.first - before.first) + abs(r.second - before.second) <= 1)) END("Yes");
