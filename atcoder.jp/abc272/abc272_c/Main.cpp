...
struct Solver {
    void solve() {
        ...
        sort(all(odd));
        sort(all(even));
        if(odd.size() <= 1 && even.size() <= 1) END("-1");
        if(odd.size() == 1 && *even.rbegin() == 0) END("-1");
        i64 ans = 0;
        if(odd.size() > 1) chmax(ans, odd[odd.size() - 1] + odd[odd.size() - 2]);
        if(even.size() > 1) chmax(ans, even[even.size() - 1] + even[even.size() - 2]);
        print(ans);
    }
};
