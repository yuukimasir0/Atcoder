...
struct Solver {
    void solve() {
        INT(W, N, K);
        vc(pint, s, N + 1);
        vector dp(N + 1, vector<vector<int>>(K + 1, vector<int>(W + 1, 0)));
        rep(N) input(s[i + 1]);
        rep(i, 1, N + 1) {
            auto &[w, val] = s[i];
            rep(k, 1, K + 1) rep(j, W + 1) {
                chmax(dp[i][k][j], dp[i - 1][k][j]);
                if(j >= w) chmax(dp[i][k][j], dp[i - 1][k - 1][j - w] + val);
            }
        }
        int ans = 0;
        rep(K + 1) chmax(ans, max(dp[N][i]));
        print(ans);
    }
};
