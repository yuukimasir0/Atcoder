...
struct Solver {
    void solve() {
        ...
        dp[1][0] = a[1][0];
        dp[1][1] = a[1][1];
        dp[2][0] = max({dp[1][1], dp[1][0]}) + a[2][0];
        dp[2][1] = max({dp[1][1], dp[1][0]}) + a[2][1];
        dp[2][2] = dp[0][1] + a[2][2];
        dp[3][0] = max({dp[2][1], dp[2][0]}) + a[3][0];
        dp[3][1] = max({dp[2][1], dp[2][0], dp[1][2]}) + a[3][1];
        dp[3][2] = max({dp[2][1], dp[2][2]}) + a[3][2];
        dp[3][3] = dp[1][2] + a[3][3];
        dp[4][0] = max({dp[3][1], dp[3][0]}) + a[4][0];
        dp[4][1] = max({dp[3][1], dp[3][0], dp[3][2]}) + a[4][1];
        dp[4][2] = max({dp[3][1], dp[3][2], dp[3][3]}) + a[4][2];
        dp[4][3] = max({dp[3][2], dp[3][3]}) + a[4][3];
        dp[4][4] = dp[3][3] + a[4][4];
        ...
        rep(i, 5, T + 1) {
            ...
            dp[i][3] = max({dp[i - 1][2], dp[i - 1][3], dp[i - 1][4]}) + a[i][3];
            dp[i][4] = max({dp[i - 1][3], dp[i - 1][4]}) + a[i][4];
        }
        auto ans = *max_element(all(dp[T]));
        print(ans);
    }
};
