...
struct Solver {
    void solve() {
        ...
        VC(i64, A, N);
        VC(string, S, N);
        vv(int, dp, N + 1, N + 1, INT_MAX);
        vv(i64, G, N + 1, N + 1);
        rep(N) rep(j, N) if(S[i][j] == 'Y') {
            dp[i + 1][j + 1] = 1;
            G[i + 1][j + 1] = A[i] + A[j];
        }
        rep(N) rep(j, N) rep(k, N) {
            if(dp[j + 1][i + 1] == INT_MAX || dp[i + 1][k + 1] == INT_MAX) continue;
            if(dp[j + 1][k + 1] == dp[j + 1][i + 1] + dp[i + 1][k + 1]) chmax(G[j + 1][k + 1], G[j + 1][i + 1] + G[i + 1][k + 1] - A[i]);
            if(chmin(dp[j + 1][k + 1], dp[j + 1][i + 1] + dp[i + 1][k + 1])) {
                G[j + 1][k + 1] = G[j + 1][i + 1] + G[i + 1][k + 1] - A[i];
            }
        }
        debugvc(dp);
        INT(Q);
