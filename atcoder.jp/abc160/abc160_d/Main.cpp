...
struct Solver {
    void solve() {
        INT(N, X, Y);
        vv(int, dist, N + 1, N + 1);
        rep(i, 1, N + 1) rep(j, 1, N + 1) dist[i][j] = abs(i - j);
        dist[X][Y] = 1;
        dist[Y][X] = 1;
        rep(i, 1, N + 1) rep(j, 1, N + 1) chmin(dist[i][j], dist[i][X] + dist[Y][j] + 1);
        map<int, int> mp;
        rep(i, 1, N + 1) rep(j, i + 1, N + 1) mp[dist[i][j]]++;
        // cout << dist;
        rep(N - 1) print(mp[i + 1]);
    }
};
