...
struct Solver {
    void solve() {
        LL(N, M, Q);
        vc<i64> a(Q), b(Q), c(Q), d(Q);
        rep(Q) input(a[i], b[i], c[i], d[i]);
        i64 ans = 0;
        auto f = [&](auto f, int num, vc<int> weight, int temp) -> void {
            rep(Q) if(num == b[i] && weight[b[i]] - weight[a[i]] == c[i]) temp += d[i];
            if(num != N)
                rep(M + 1) {
                    weight[num + 1] = weight[num] + i;
                    if(weight[num + 1] > M) break;
                    f(f, num + 1, weight, temp);
                }
            if(num == N) {
                debugvc(weight);
                debug(temp);
