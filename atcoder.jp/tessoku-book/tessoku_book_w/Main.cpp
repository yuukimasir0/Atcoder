...
// clang-format on
constexpr long long LL_INF = 0x4fffffffffffffffLL;
constexpr int INT_INF = 0x4fffffff;
...
struct Solver {
    void solve() {
        INT(N, M);
        vv(int, A, M + 1, N + 1);
        rep(M) rep(j, N) input(A[i + 1][j + 1]);
        vv(int, dp, M + 1, 1 << N, INT_INF);
        dp[0][0] = 0;
        rep(i, 1, M + 1) rep(s, 1 << N) {
            vc(int, tmp, N + 1);
            rep(j, 1, N + 1) {
                if(s & (1 << (j - 1)))
                    tmp[j] = 1;
            ...
            int st = 0;
            rep(j, 1, N + 1) if(tmp[j] || A[i][j]) st += (1 << (j - 1));
            chmin(dp[i][s], dp[i - 1][s]);
            chmin(dp[i][st], dp[i - 1][s] + 1);
        }
        printif(dp[M][(1 << N) - 1] == INT_INF, "-1", dp[M][(1 << N) - 1]);
    }
};
