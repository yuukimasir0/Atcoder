...
struct Solver {
    void solve() {
        ...
        rep(m + 1) {
            bool tmp[3] = {};
            if(i >= X) tmp[grundy[i - X]] = true;
            if(i >= Y) tmp[grundy[i - Y]] = true;
            if(tmp[0] == false) grundy[i] = 0;
            else if(tmp[1] == false) grundy[i] = 1;
            else grundy[i] = 2;
        }
        int xored = 0;
        rep(N) xored ^= grundy[A[i]];
        printif(xored, "First", "Second");
    }
};
