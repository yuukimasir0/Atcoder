...
struct Solver {
    void solve() {
        ...
        rep(N) input(P[i + 1]);
        vv(i64, dp, N + 1, N + 1);
        rrep(i, N - 1) rep(j, 1, N - i + 1) {
            int k = i + j;
            int tmp1 = 0, tmp2 = 0;
            if(j - 1 >= 0 && j <= P[j - 1].first && P[j - 1].first <= k) tmp1 = P[j - 1].second;
            if(k + 1 <= N && j <= P[k + 1].first && P[k + 1].first <= k) tmp2 = P[k + 1].second;
            if(j == 1)
                dp[j][k] = dp[j][k + 1] + tmp2;
            else if(k == N)
                dp[j][k] = dp[j - 1][k] + tmp1;
            else
                dp[j][k] = max(dp[j - 1][k] + tmp1, dp[j][k + 1] + tmp2);
        }
        i64 ans = 0;
        rep(i, 1,N + 1) chmax(ans, dp[i][i]);
        print(ans);
    }
};
