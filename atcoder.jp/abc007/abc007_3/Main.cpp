...
struct Solver {
    void solve() {
        LL(R, C);
        vc<vv<pair<i32, i32>>> g(R + 1, vv<pair<i32, i32>>(C + 1));
        vv<char> c(R + 1, vc<char>(C + 1));
        vc<pair<i32, i32>> d = {
            { 0,  1},
            { 0, -1},
            { 1,  0},
        ...
        pair<i32, i32> S, G;
        input(S, G);
        rep(i, 1, R + 1) rep(j, 1, C + 1) input(c[i][j]);
        rep(i, 1, R + 1) rep(j, 1, C + 1) if(c[i][j] == '.') 
            for(const auto &[dx, dy] : d) 
                if(i + dx >= 0 && i + dx <= R && j + dy >= 0 && j + dy <= C && c[i + dx][j + dy] == '.') {
                    g[i][j].emplace_back(pair(i + dx, j + dy));
                    g[i + dx][j + dy].emplace_back(pair(i, j));
                }
        vv<i32> dist(R + 1, vc<i32>(C + 1, INT32_MAX));
        ...
        while(q.size()) {
            auto &[vx, vy] = q.front();
            q.pop();
            for(const auto &[ux, uy] : g[vx][vy])
                if(chmin(dist[ux][uy], dist[vx][vy] + 1)) {
                    q.push(pair(ux, uy));
                }
        }
        print(dist[G.first][G.second]);
    }
};
